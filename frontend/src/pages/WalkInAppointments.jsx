  ...
  const loadAll = async () => {
    ...
    try {
      const [
        ...
        fetchWalkinKpis(filters || {}),
        fetchWalkinsTrend({ ...(filters || {}), dateType: trendType }),
        fetchWalkinsByDepartment(filters || {}),
        fetchWalkinsByService(filters || {}),
      ]);
  ...
  {byService.length === 0 ? (
    ...
    <ResponsiveContainer width="100%" height={320}>
      <BarChart data={byService}>
        <CartesianGrid strokeDasharray="3 3" />
        <XAxis dataKey="service_name" angle={-10}
  textAnchor="end"
  interval={0}/>
        <YAxis />
        <Tooltip />
