...
export default function OfficerForm() {
  ...
  const fetchOfficer = async () => {
    try {
      const res = await getUserByEntityId(officer_id);

      if (!res?.data?.success) {
        Swal.fire("Error", "Failed to load officer details", "error");
        return;
      }

      const o = res.data.data;

      console.log("🧾 Officer Data:", o);
      ...
      setForm((prev) => ({
        ...
        officer_address: o.officer_address || "",
        officer_state_code: o.officer_state_code || "",
        officer_division_code: o.officer_division_code || "",
        officer_district_code: o.officer_district_code || "",
        officer_taluka_code: o.officer_taluka_code || "",
        officer_pincode: o.officer_pincode || "",
        photo: null, // ⚠️ never prefill file input
      }));

      /* ================= LOAD CASCADES ================= */
      if (o.officer_state_code) {
        await fetchDivisions(o.officer_state_code);
      }

      if (o.officer_state_code && o.officer_division_code) {
      ...
      if (o.organization_id) {
        await fetchDepartments(o.organization_id);
      }

    } catch (error) {
      console.error("❌ Failed to fetch officer:", error);
      Swal.fire("Error", "Server error while loading officer", "error");
    }
  };
...
const handleSubmit = async (e) => {
  ...
  // 🔐 Password validation (ONLY if user entered)
  ...
  try {
    ...
    // ✅ entity_id REQUIRED for update
    formData.append("entity_id", officer_id);  // from params
    ...
    // ❌ If no photo selected, DO NOT append it
    if (!form.photo) {
      formData.delete("photo");
    }

    // ❌ If password not updated, do not send it
    if (!form.password) {
      formData.delete("password");
    }

    const res = await updateOfficerByRole(formData);

    if (res?.data?.success !== true) {
      Swal.fire({
        icon: "error",
        title: "Update Failed",
        text: res?.data?.message || "Unable to update officer",
      });
      return;
    }
    ...
    Swal.fire({
      ...
      text: res?.data?.message || "Officer updated successfully",
    });
    ...
    Swal.fire({
      icon: "error",
      title: "Server Error",
      text: err?.response?.data?.message || "Unable to update officer",
    });
  } finally {
    setSubmitting(false);
  }
